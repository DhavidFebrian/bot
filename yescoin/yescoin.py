...
█▄█ █▀█ █▀█ █▄▄ █ █▄█ █ ██▄
    ...
    print(Fore.YELLOW + Style.BRIGHT + "Free Konsultasi Join Telegram Channel: https://t.me/FullScriptTelegram")
    print(Fore.BLUE + Style.BRIGHT + " BOT CREATED BY DHAVID FEBRIAN VALENTINO")
...
def process_account(token):
    ...
    if squad_info and squad_info['data']['isJoinSquad']:
        ...
        print(f"{Fore.YELLOW + Style.BRIGHT}\r[ Squad ] : Belum Join Squad. Joining AirdropXID.", end="", flush=True)
        time.sleep(3)
        join_result = join_squad(token, "t.me/AirdropFree_XID")
        if join_result:
            print(f"{random.choice(available_colors) + Style.BRIGHT}\r[ Squad ] : Welcome Pemulung {nickname} - AIRDROPERS !      ", flush=True)
        else:
            print(f"{random.choice(available_colors) + Style.BRIGHT}\r[ Squad ] : Failed to join AirdropXID.", flush=True)
